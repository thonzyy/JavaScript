...
admin이면 읽기, 쓰기, 삭제를 출력, type이 없는 경우에는 "접속 형태를 선택해주세요"를 출력
-->
<c:choose>
<c:when test="${param.type eq '' }">접속자를 선택해주세요</c:when>
<c:when test="${param.type eq 'guest' }">읽기</c:when>
<c:when test="${param.type eq 'user' }">읽기,쓰기</c:when>
<c:when test="${param.type eq 'admin' }">읽기,쓰기,삭제</c:when>
</c:choose>

</div>

<div style="width: 338px">
<!-- jstl_if.jsp에서 name에 tk가 입력된 후 전송이 눌려지면 scope 객체 중 하나를 선택하여
값을 넣은 다음 "이동"이 눌려졌을 때 아래  리뷰작성 버튼이 보여질 수 있도록 
 -->
 <%
 %>
