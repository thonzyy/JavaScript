...
$(function(){
	...
	$("#btn").click(function(){
		if($("#idDupFlag").val()==0){
			alert($("#id").val()+"은 중복확인 되지 않은 아이디입니다. \n중복확인을 수행해주세요.");
			return;
		}
		$("#frm").submit();
...
<%
	...
	try{
		...
		pageContext.setAttribute("member", wmVO);//스콥 객체가 try안에 있어도 바깥에서 사용할 수 있다.
		...
		<form action="member_update_process.jsp" name="frm" id="frm"  method="post">
			<table>
				<tr>
					...
					<td>
						<input  name="id" id="id" class="inputTxt inputIdtype" 
						 readonly="readonly"  value=" ${ member.id }" type="text" maxlength="20"  />
					</td>
				</tr>
				...
				<tr>
					...
					<td class="mail_type">
						<input data-value="이메일을 입력해주세요." name="email1"
						value="${ member.email }" 
				...
				<tr>
					...
					pageContext.setAttribute("area", area);%>
						<select id="area" name="area">
							...
							<c:forEach var="area" items="${area }" varStatus="i"><option value="${i.count }" ${i.count eq member.area? "selected='selected'":"" }><c:out value="${area }"/></option>
							</c:forEach>
...
<!-- 						<input  id="area" name="area" class="inputTxt inputName" type="text" /> -->
				...
				<tr>
					...
					<td>
						<input type="text" class="zipcode" id="zipcode" name="zipcode" value="${member.zipcode}" readonly="readonly" />
