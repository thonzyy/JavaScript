	...
	$.ajax({
		...
		success:function(jsonObj){
			...
			if(jsonObj.dataEMpty){
				output+="<div>해당 글에는 댓글이 없습니다.<br/><img src='../common/images/error.jpg' width='100'/></div>";
		...
		if(!jsonObj.dataEmpty){
			$.each(jsonObj.data,function(i,json){
				output+="<div id='reply_"+json.reply_num+"' class='reply'>"+json.content+" / "+json.input_date+"<br>"
				+json.writer+"<a href='#void' onclick='removeReply("+json.reply_num+")' class='btn btn-warning'>삭제</a></div>"
			})//each
		}//end if 
		$("#output").html(output);
	}
	...
	$.ajax({
		...
		success:function(jsonObj){
			if(jsonObj.deleteFlag){
				...
				if($(".reply").length==0) {
					var output= "<div>해당 글에는 댓글이 없습니다.<br/><img src='../common/images/error.jpg' width='100'/></div>";
					$("#output").html(output);
				}//end if
			}else {
				alert("리플 삭제 실패")
